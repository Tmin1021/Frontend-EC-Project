...
export default function Admin_Universal_Item({which, info=null, header=0, lastRow=false}) {
  ...
  const main_infos = {
    inventory:  {product_id: ["Product ID", info?.norm_id], name: ["Name", info?.name], price: ['Price', info?.price], dynamicPrice: ["Dynamic", info?.dynamicPrice],
                 stock: ['Stock', info?.stock], sales_count: ["Sold", info?.sales_count], available: ['Status', info?.available]},
    user:       {user_id: ["User ID", info?.norm_id], name: ["Name", info?.name], email: ["Email", info?.email],
                 phone: ['Phone', info?.phone], address: ['Address', info?.address], role: ['Role', info?.role]},
    order:      {order_id: ['Order ID', info?.norm_id], orderer: ['Orderer', info?.user_name], order_date: ['Order date', info?.order_date?.toString().slice(0,10)],
                 shipping_address: ['Shipping adress', info?.shipping_address], total: ['Total', getRoundPrice(info?.subtotal - info?.off_price)], status: ['Status', info?.status]}, 
    order_item: {product_id: ['Product ID', info?.product_id], name: ['Name', info?.product_name], subtotal: ['Subtotal', info?.subtotal],
                 quantity: ['Quantity', info?.quantity], off_price: ['Off price', info?.off_price], total: ['Total', getRoundPrice(info?.subtotal-info?.off_price)]
    },
    order_item_lastRow: {product_id: ['Product ID', ''], name: ['Name', ''], subtotal: ['Subtotal', info?.subtotal],
                 quantity: ['Quantity', info?.quantity], off_price: ['Off price', info?.off_price], total: ['Total', getRoundPrice(info?.subtotal-info?.off_price)]}          
    }
