...
function Option_Item({name, content}) {
    ...
    return (
      <div className={` ${isClick? 'z-55 fixed inset-0 flex justify-center items-center bg-black/20 backdrop-blur-sm':''} transition-all`} onClick={()=>setIsClick(false)}>
          <div className={`${isClick ? 'w-[60%] md:w-[30%] bg-white/80 p-4 rounded-lg max-h-[80vh] overflow-y-auto':'max-w-[200px] w-full h-[140px]'} cursor-pointer flex flex-col justify-start items-center py-5 rounded-sm shadow-sm border-1 border-gray-300 hover:shadow-lg transition-all`}
              ...
              {!isClick && content.length > 3 && <p className='font-semibold text-sm text-blue-400 mt-1'>View More</p>   }
