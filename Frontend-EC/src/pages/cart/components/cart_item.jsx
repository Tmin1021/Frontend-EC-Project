        ...
        <div className='h-[100px] w-full md:h-[120px] px-1 md:p-4 flex gap-2'>
            ...
            <div className='flex flex-col justify-between w-full'>
                ...
                <div className='flex gap-2'>
                    ...
                    {product.option && <div className='text-xs font-semibold text-gray-500/80 backdrop-blur-xs bg-gray-100 p-1 rounded-sm w-fit'>{product?.option?.name}</div>}
                    {product.option && <div className='text-xs font-semibold text-gray-500/80 backdrop-blur-xs bg-gray-100 p-1 rounded-sm w-fit'>{product?.option?.stems} stems</div>}
                </div>

                {/* Price */}
                <div className='flex items-center gap-4 w-full'>
                    <p className='text-red-500/80 font-semibold text-lg w-[50px] md:w-[80px]'>{Math.round(100*((product.option?.price ?? product.product.price) * product.quantity-product.off_price))/100}</p>
                    {product.off_price!==0 && <div className='flex items-center gap-1'>
                        <TicketPercent className='text-blue-500'/>
                        <p className='line-through text-sm font-medium text-gray-500 '>{Math.round(100*((product.option?.price ?? product.product.price) * product.quantity))/100}</p>
                    </div>}
                </div>
                {/* Date 
